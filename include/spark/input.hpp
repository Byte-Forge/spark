...
namespace spark
{
	...
	{
	public:
		static inline void SetKeyState(const int key, int action, const int mods) { action = TranslateState(action); m_keyInputs[(KeyboardCode)key] = action; std::cout << key << std::endl; }
